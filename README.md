...
For the `HashKey64` type, we set $N$ to $2^{64}$, corresponding to wrappped
arithmetic with `u64` values.  We provide an array `HIST_CHAR_VAL` for the
character values $u(c)$; these were generated by a pseudo-random number
generator.
